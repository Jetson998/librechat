...
SERVER_PATH="api/server/index.js"
SERVER_BLOB="f88b2fd2581bb612179b6d0800e8c5ef9287858a"
...
test "$(git -C "$UPSTREAM_DIR" rev-parse "$PIN:$SERVER_PATH")" = "$SERVER_BLOB"
test -f "$PATCH_PATH"
...
node --check "$VERIFY_DIR/$SERVER_PATH"
...
test "$(rg -c '@jetson998/librechat-file-agent-host-installer/v1' "$VERIFY_DIR/$SERVER_PATH")" = "1"
if rg -q 'process\.env\..*FILE_AGENT' "$VERIFY_DIR/$SERVER_PATH"; then
  exit 1
fi
test "$(git -C "$VERIFY_DIR" diff --name-only | wc -l | tr -d ' ')" = "3"
git -C "$VERIFY_DIR" diff --name-only | sort | diff -u - <(printf '%s\n%s\n%s\n' "$SERVER_PATH" "$SOURCE_PATH" "$ROUTE_PATH" | sort)

printf 'upstream_pin=%s\n' "$PIN"
printf 'source_blob=%s\n' "$SOURCE_BLOB"
printf 'route_blob=%s\n' "$ROUTE_BLOB"
printf 'server_blob=%s\n' "$SERVER_BLOB"
