...
LEGACY_CONTACT = {"name": "LibreChat Workflow Agent", "email": ""}


def validate_contact_scope(compiled: dict, snapshot: dict) -> None:
    expected_ids = {agent["id"] for agent in compiled["agents"]}
    agents = snapshot.get("agents", [])
    actual_ids = {agent.get("id") for agent in agents}
    if actual_ids != expected_ids or len(agents) != 7:
        raise RuntimeError("preflight must find the exact seven preset Agents")
    for agent in agents:
        contact = agent.get("support_contact")
        if contact is not None and contact != LEGACY_CONTACT:
            raise RuntimeError(f"{agent.get('id')} has an unexpected support_contact")


def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit(
            "usage: remote-preflight.py <compiled-agents.json> <snapshot-targets.js> <source-revision>"
        )
    ...
    validate_contact_scope(compiled, snapshot)
