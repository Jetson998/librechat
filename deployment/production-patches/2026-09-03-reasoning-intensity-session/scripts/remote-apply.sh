...
release_root="$root_dir/reasoning-intensity-session/${source_revision:0:12}-$timestamp"
release_client="$release_root/client-dist"
backup_dir="$root_dir/backups/reasoning-intensity-session-${source_revision:0:12}-$timestamp"
work_dir="$(mktemp -d /tmp/librechat-reasoning-intensity-session-apply.XXXXXX)"
...
expected_index_sha="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["client"]["index_sha256"])' "$metadata_path")"
