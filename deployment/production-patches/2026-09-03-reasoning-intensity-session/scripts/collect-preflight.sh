...
remote_stage="/tmp/librechat-reasoning-intensity-session-preflight-${source_revision:0:12}"
remote_raw="$remote_stage/runtime-raw.json"
local_raw="$(mktemp "${TMPDIR:-/tmp}/librechat-reasoning-intensity-session-runtime.XXXXXX")"
...
transport_exec "chmod 700 '$remote_stage/remote-preflight.py' && python3 '$remote_stage/remote-preflight.py' '$remote_stage/artifact.json' '$remote_raw' >/dev/null"
...
raw.update(
    {
        ...
        "client_artifact": {
            ...
            "build_provider": metadata["build_provenance"]["provider"],
            "build_environment": metadata["build_provenance"]["build_environment"],
            "build_source_revision": metadata["source"]["repository_commit"],
            "zip_sha256": metadata["artifact"]["zip_sha256"],
            "client_tar_sha256": metadata["artifact"]["members"][
                "client-dist.tar.gz"
            ],
            "client_index_sha256": metadata["client"]["index_sha256"],
            "feature_contract": metadata["client"]["required_markers"],
        },
    }
)
