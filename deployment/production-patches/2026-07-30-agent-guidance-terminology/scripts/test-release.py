#!/usr/bin/env python3
"""Contracts for the bounded Agent guidance and terminology Client release."""
...
def load_verifier():
    ...
    spec = importlib.util.spec_from_file_location("agent_guidance_terminology_verifier", path)
...
def main() -> None:
    ...
    check(
        metadata["component"] == "agent-guidance-terminology-client",
    ...
    check(len(metadata["artifact"]["members"]) == 7, "artifact member contract changed")
    check(
    ...
    check("agent-guidance-terminology" in apply, "versioned Client destination missing")
    check(
    ...
    check(
        'librechat-agent-guidance-terminology-runtime.XXXXXX"' in collect
        and "librechat-agent-guidance-terminology-runtime.XXXXXX." not in collect,
    ...
    print("agent_guidance_terminology_release_contract=passed")
