...
const testBoundedResponseShape = () => {
  const doc = service.normalizeDiagnosticEvent({
  ...
  assert.deepEqual(doc.responseShape, {
    completionKind: 'array',
    completionParts: 64,
  ...
  assert(!JSON.stringify(doc).includes('private-prompt-fragment'));
  assert(!JSON.stringify(doc).includes('must-not-be-stored'));
};
