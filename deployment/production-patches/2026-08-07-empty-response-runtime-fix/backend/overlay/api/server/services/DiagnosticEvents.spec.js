...
describe('DiagnosticEvents', () => {
  ...
  it('stores only bounded response-shape metadata for an empty model response', () => {
    const doc = normalizeDiagnosticEvent({
      event: 'generation_failed',
      responseShape: {
        completionKind: 'array',
        completionParts: 999,
        completionPartTypes: ['text', 'tool_call', 'private-prompt-fragment'],
        textLength: 9_999_999,
        nonBlankTextParts: 999,
        metadataKind: 'object',
        metadataKeys: ['usage', 'provider', 'api_key'],
        responseFields: ['content', 'metadata', 'prompt'],
        secret: 'must-not-be-stored',
      },
    });

    expect(doc.responseShape).toEqual({
      completionKind: 'array',
      completionParts: 64,
      completionPartTypes: ['text', 'tool_call'],
      textLength: 1_000_000,
      nonBlankTextParts: 64,
      metadataKind: 'object',
      metadataKeys: ['usage', 'provider'],
      responseFields: ['content', 'metadata'],
    });
    expect(JSON.stringify(doc)).not.toContain('private-prompt-fragment');
    expect(JSON.stringify(doc)).not.toContain('must-not-be-stored');
  });
