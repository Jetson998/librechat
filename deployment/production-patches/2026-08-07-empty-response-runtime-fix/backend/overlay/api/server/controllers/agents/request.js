...
function recordAgentDiagnostic({
  ...
  recordDiagnosticEvent({
    ...
    messageId,
    errorCode,
    responseShape: responseShape ?? error?.responseShape,
...
function validCreatedAt(value) {
  const normalized = toValidISOString(value);
  return normalized == null ? undefined : new Date(normalized);
}

async function persistFileAgentUserTurn({
  req,
  userId,
  conversationId,
  isNewConvo,
  ...
  if (!preliminaryUserMessage?.messageId || !preliminaryResponseMessageId) {
    throw new Error('Current File Agent turn does not have stable message identities.');
  }
  ...
  // Save the conversation before the user message. A failed conversation write
  // therefore remains an ordinary initialization failure and cannot create a
  // partially persisted File Agent turn.
  const savedConversation = await saveConvo(requestContext, conversationFields, {
    context: 'api/server/controllers/agents/request.js - file agent conversation',
    createdAtOnInsert: validCreatedAt(req?.conversationCreatedAt),
  });
  if (savedConversation?.conversationId !== conversationId) {
    throw new Error('File Agent conversation persistence was not confirmed.');
  }
  ...
  const savedUserMessage = await saveMessage(requestContext, userMessage, {
    context: 'api/server/controllers/agents/request.js - file agent user message',
  });
  try {
    ...
    // The durable conversation/message pair is authoritative for a Runtime
    // delivery. Metadata only improves resumption and must not reverse a
    // successful handoff after user persistence.
    logger.warn('[ResumableAgentController] File Agent metadata update failed', {
      streamId,
      name: error?.name ?? 'Error',
    });
  }
  // Runtime-backed turns bypass AgentClient.sendMessage(), which normally emits
  // this event from onStart. Keep the client-side turn contract identical while
  // making the durable conversation/message pair authoritative first.
  try {
    ...
    // A later final event still rehydrates the persisted turn; a transient SSE
    // delivery failure must not create a second user message or reject a
    // durable Runtime handoff.
    logger.warn('[ResumableAgentController] File Agent created event failed', {
      streamId,
      name: error?.name ?? 'Error',
    });
  }
...
async function finalizeFileAgentHandoffFailure({
  req,
  userId,
  conversationId,
  streamId,
  ...
  const errorText = 'The file task could not be submitted. Please try again.';
  const requestContext = {
    userId,
    isTemporary: req?.body?.isTemporary,
    interfaceConfig: req?.config?.interfaceConfig,
  ...
  try {
    ...
    const savedResponseMessage = await saveMessage(requestContext, responseMessage, {
      context: 'api/server/controllers/agents/request.js - file agent handoff failure',
    });
    const conversation = persisted?.conversation ?? await getConvo(userId, conversationId);
    await GenerationJobManager.emitDone(streamId, {
      final: true,
      conversation,
      title: conversation?.title || 'New Chat',
      requestMessage: sanitizeMessageForTransmit(persisted?.userMessage ?? preliminaryUserMessage),
      responseMessage: savedResponseMessage ?? responseMessage,
      error: { message: errorText },
    });
    GenerationJobManager.completeJob(streamId);
  } catch (finalizationError) {
    logger.error('[ResumableAgentController] File Agent handoff failure finalization failed', {
      name: finalizationError?.name ?? 'Error',
      streamId,
    });
    await GenerationJobManager.emitError(streamId, errorText).catch(() => {});
    GenerationJobManager.completeJob(streamId, errorText);
  } finally {
    recordAgentDiagnostic({
      req,
      event: 'file_agent_handoff_failed',
      stage: 'file_agent_runtime',
      error,
      errorCode: 'FILE_AGENT_HANDOFF_FAILED',
      userId,
      conversationId,
      streamId,
      messageId: preliminaryResponseMessageId,
    ...
    await finishResumableRequest(req, userId).catch((cleanupError) => {
      logger.error('[ResumableAgentController] File Agent handoff cleanup failed', {
...
const ResumableAgentController = async (
  ...
  initializeClient,
  addTitle,
  fileAgentRuntimeBridge = null,
) => {
  req.diagnosticStartedAt ??= Date.now();
  const {
    text,
    isRegenerate,
    endpointOption,
  ...
  if (
    ...
    recordAgentDiagnostic({
      req,
      event: 'followup_rejected_parent_saving',
      stage: 'followup',
      userId,
      conversationId: reqConversationId,
      streamId: req.body?.streamId ?? reqConversationId,
  ...
  try {
    ...
    const runtimeBridge = typeof fileAgentRuntimeBridge?.tryRoute === 'function'
      ? fileAgentRuntimeBridge
      : null;
    if (runtimeBridge) {
      let runtimeHandoff;
      try {
        runtimeHandoff = await runtimeBridge.tryRoute({
          req,
          res,
          client,
          userId,
          ...
          streamId,
          text,
          persistUserTurn: () => persistFileAgentUserTurn({
            req,
            userId,
            conversationId,
            isNewConvo,
        ...
        if (error?.userTurnPersisted === true) {
          await finalizeFileAgentHandoffFailure({
            req,
            userId,
            conversationId,
            streamId,
      ...
      if (runtimeHandoff?.suppressNativeAgent === true) {
        try {
          await GenerationJobManager.updateMetadata(streamId, {
            fileAgentDeliveryId: runtimeHandoff.deliveryId,
            fileAgentTaskId: runtimeHandoff.taskId ?? undefined,
          });
        } catch (error) {
          logger.warn('[ResumableAgentController] File Agent delivery metadata update failed', {
            streamId,
            name: error?.name ?? 'Error',
          });
        }
    ...
    const startGeneration = async () => {
      ...
      try {
        ...
        // Only the job's abort signal identifies a user/system cancellation. Do
        // not classify provider error text containing "abort" as a user Stop.
        ...
        if (wasAborted) {
          ...
          recordAgentDiagnostic({
            ...
            conversationId,
            streamId,
            messageId: req.body?.responseMessageId ?? req.body?.messageId,
            endpointOption,
          });
          let terminalFailurePersisted = false;
          const failureUserMessage = userMessage ?? preliminaryUserMessage;
          const failureResponseMessageId =
            client?.responseMessageId ?? preliminaryResponseMessageId;
          if (res.headersSent && failureUserMessage && failureResponseMessageId) {
            try {
              const failureState = await persistInitializationFailure({
                ...
                endpointIconURL,
                responseModel,
                preliminaryUserMessage: failureUserMessage,
                preliminaryResponseMessageId: failureResponseMessageId,
                error,
                saveMessage,
                saveConvo,
              });
              await GenerationJobManager.emitDone(streamId, {
                ...
                error: {
                  ...
                  message: failureState.responseMessage?.text || 'Generation failed',
                },
              });
              ...
              logger.error(
                '[ResumableAgentController] Failed to persist generation terminal state:',
                persistError,
              );
            }
          }
...
const _LegacyAgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    ...
    recordAgentDiagnostic({
      ...
      conversationId,
      streamId,
      messageId: responseMessageId ?? userMessageId,
      endpointOption,
    });
