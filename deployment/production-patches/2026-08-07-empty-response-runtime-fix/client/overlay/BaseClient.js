...
} = require('~/server/services/Files/GeneratedArtifactVisibility');
...
const isGeneratedFileAttachment = (file) =>
  Boolean(
    file &&
      !TOOL_ATTACHMENT_KEYS.includes(file.type) &&
      (file.filename || file.name) &&
      (file.file_id || file.filepath),
  );

const isDownloadableMessageFile = (file) =>
  Boolean(file?.file_id && isGeneratedFileAttachment(file));
...
const RESPONSE_SHAPE_TEXT_FIELDS = new Set([
  'text',
  'value',
  'content',
  'summary',
...
const RESPONSE_SHAPE_METADATA_KEYS = [
  'id',
  'model',
  'type',
  'status',
...
const RESPONSE_SHAPE_RESPONSE_FIELDS = [
  'text',
  'content',
  'summary',
  'files',
...
const getObjectKeysFromAllowlist = (value, allowlist) => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return [];
  }
  return allowlist.filter((key) => Object.prototype.hasOwnProperty.call(value, key));
};

const collectResponseTextStats = (value, stats, key, depth = 0, visited = new WeakSet()) => {
  if (depth > 4 || stats.nodes >= 128 || value == null) {
    return;
  }

  if (typeof value === 'string') {
    if (!RESPONSE_SHAPE_TEXT_FIELDS.has(key)) {
      return;
    }
    stats.nodes += 1;
    stats.textLength += value.length;
    if (isNonBlankText(value)) {
      stats.nonBlankTextParts += 1;
    }
    return;
  }
  ...
  if (Array.isArray(value)) {
    for (const item of value.slice(0, 64)) {
      collectResponseTextStats(item, stats, key, depth + 1, visited);
    }
    return;
  }

  for (const [childKey, childValue] of Object.entries(value).slice(0, 64)) {
    collectResponseTextStats(childValue, stats, childKey, depth + 1, visited);
  }
};

const buildResponseShape = ({ completion, metadata, responseMessage }) => {
  const stats = { nodes: 0, textLength: 0, nonBlankTextParts: 0 };
  collectResponseTextStats(completion, stats, 'content');
  const completionPartTypes = Array.isArray(completion)
    ? Array.from(
        new Set(
          completion
            .slice(0, 64)
            .map((part) => (part && typeof part === 'object' ? part.type : undefined))
            .filter((type) => RESPONSE_SHAPE_PART_TYPES.has(type))
            .slice(0, 16),
        ),
      )
    : [];
  ...
  return {
    completionKind: Array.isArray(completion) ? 'array' : typeof completion,
    completionParts: Array.isArray(completion) ? completion.length : undefined,
    completionPartTypes,
    textLength: stats.textLength,
    nonBlankTextParts: stats.nonBlankTextParts,
    metadataKind: Array.isArray(metadata) ? 'array' : typeof metadata,
    metadataKeys: getObjectKeysFromAllowlist(metadata, RESPONSE_SHAPE_METADATA_KEYS),
    responseFields: getObjectKeysFromAllowlist(responseMessage, RESPONSE_SHAPE_RESPONSE_FIELDS),
  };
};
...
const createEmptyModelResponseError = (responseShape) => {
...
const ensureAssistantSemanticContent = (message, responseShape) => {
  if (!hasAssistantSemanticContent(message)) {
    throw createEmptyModelResponseError(responseShape);
  }
  return message;
};
...
class BaseClient {
  ...
  async sendMessage(message, opts = {}) {
    ...
    if (this.artifactPromises) {
      ...
      const displayOnlyAttachments = artifactAttachments.filter(
        (attachment) => !isGeneratedFileAttachment(attachment),
      );
      const generatedFiles = artifactAttachments.filter(isGeneratedFileAttachment);
      const requestText = [
        userMessage?.text,
        ...
        .filter(Boolean)
        .join('\n');
      const { visible, hidden, overflow } = selectVisibleGeneratedArtifacts(generatedFiles, {
        requestText,
      });

      if (hidden.length > 0 || overflow.length > 0) {
        logger.info('[BaseClient] Generated artifact visibility applied', {
          messageId: responseMessage.messageId,
          conversationId: responseMessage.conversationId,
          generatedCount: generatedFiles.length,
          visibleCount: visible.length,
          hiddenInternalCount: hidden.length,
          hiddenOverflowCount: overflow.length,
          visibleFiles: visible.map((file) => file.filename || file.name || file.filepath),
        });
      }

      responseMessage.attachments = [...displayOnlyAttachments, ...visible];
    ...
    try {
      ensureAssistantSemanticContent(
        responseMessage,
        buildResponseShape({ completion, metadata, responseMessage }),
      );
    } catch (error) {
