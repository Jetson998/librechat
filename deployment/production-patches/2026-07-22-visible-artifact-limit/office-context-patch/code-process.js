...
const processCodeOutput = async ({
  ...
  try {
    ...
    if (isImage) {
      ...
      const artifactClassification = classifyGeneratedArtifact({
        filename: safeName,
      ...
      const file = {
        ...
        metadata: {
          codeEnvRef,
          artifactRole: artifactClassification.role,
          artifactVisibilityReason: artifactClassification.reason,
        },
 ...
 * @returns {Promise<{
 * files: Array<{ id: string; source_file_id: string; session_id: string; name: string }>,
 * toolContext: string,
 * }>}
 */
