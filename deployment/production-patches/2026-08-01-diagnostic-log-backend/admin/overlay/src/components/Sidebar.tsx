...
import { READ_DIAGNOSTIC_LOGS_CAPABILITY, SystemCapabilities } from '@/constants';
...
const navItems: t.NavItem[] = [
  { labelKey: 'com_nav_dashboard', path: '/', icon: 'home' },
  {
    ...
    path: '/logs',
    icon: 'document',
    capability: READ_DIAGNOSTIC_LOGS_CAPABILITY,
  },
  {
  ...
  return (
    <>
      <aside
        ...
        className={cn(
          'z-(--z-floating) border-(--cui-color-stroke-default) bg-(--cui-color-background-panel) sticky top-0 flex h-screen shrink-0 flex-col overflow-hidden border-r transition-[width] duration-200',
          collapsed ? 'w-14' : 'w-63',
        )}
      >
        <div className="flex h-14 shrink-0 items-center px-2">
          <div className="flex items-center gap-2.5 overflow-hidden px-1.5">
            ...
            />
            <span className="text-(--cui-color-text-default) truncate text-sm font-semibold">
          ...
          <div className="flex flex-col gap-0.5">
            {visibleItems.map((item) => (
              <Link
                ...
                className={cn(
                  'flex h-8 items-center gap-2.5 overflow-hidden whitespace-nowrap rounded-md px-2.5 text-sm no-underline transition-colors duration-100',
                  isActive(item.path)
                    ? 'bg-(--cui-color-background-active) text-(--cui-color-text-default) font-medium'
                    : 'text-(--cui-color-text-muted) hover:bg-(--cui-color-background-hover) hover:text-(--cui-color-text-default) font-normal',
                )}
              >
        ...
        {initials && (
          <div className="border-(--cui-color-stroke-default) flex shrink-0 items-center border-t px-2 py-3">
            <div className="flex items-center gap-2.5 overflow-hidden px-0.5">
              <Dropdown>
                <Dropdown.Trigger>
                  <button
                    ref={userMenuRef}
                    type="button"
                    className="border-(--cui-color-stroke-default) bg-(--cui-color-background-default) hover:border-(--cui-color-stroke-intense) hover:bg-(--cui-color-background-hover) flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-full border transition-colors"
                  ...
                  >
                    <span
                      aria-hidden="true"
                      className="text-(--cui-color-text-muted) text-xs font-medium"
                    >
                      {initials}
                    </span>
                  </button>
                ...
                <Dropdown.Content>
                  <div className="user-dropdown min-w-45 flex select-none flex-col">
                    {user && (
                      <div className="px-3 pb-2 pt-3">
                        <span className="text-(--cui-color-text-default) block text-sm font-medium leading-tight">
                          {user.name || ''}
                        </span>
                        {user.email && (
                          <span className="text-(--cui-color-text-muted) mt-0.5 block text-xs leading-tight">
                            {user.email}
                          </span>
                        )}
                      </div>
                    )}
                    <div className="border-(--cui-color-stroke-default) border-t" />
              ...
              {user && (
                <div className="min-w-0 flex-1">
                  <span className="text-(--cui-color-text-default) block truncate text-sm font-medium leading-tight">
                    {user.name || ''}
                  </span>
                  {user.email && (
                    <span className="text-(--cui-color-text-muted) block truncate text-xs leading-tight">
                      {user.email}
                    </span>
                  )}
                </div>
        ...
        <button
          ...
          className="border-(--cui-color-stroke-default) text-(--cui-color-text-muted) hover:bg-(--cui-color-background-hover) hover:text-(--cui-color-text-default) flex w-full shrink-0 cursor-pointer items-center justify-center border-t bg-transparent py-3 transition-colors"
        >
