...
expected_prompt="根据用户的首条消息生成简洁、准确的会话标题。只输出标题本身，不要解释，不要提问，不要使用引号，不要添加“标题：”等前缀。标题应概括核心主题，中文最多20个汉字；如果原内容不是中文，则使用原内容对应的语言。"
...
db.codexConfigBackups.insertOne({
  backupId: '$backup_id', reason: 'MuskAPI conversation title configuration',
