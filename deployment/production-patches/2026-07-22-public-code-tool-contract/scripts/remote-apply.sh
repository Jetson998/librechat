...
release_dir="/opt/librechat/public-code-tool-contract/${source_revision:0:12}-$timestamp"
compose_dir="/opt/librechat"
compose_override="$compose_dir/compose.override.yaml"
compose_backup="$compose_override.bak-$timestamp"
backup_id="public-code-tool-contract-${source_revision:0:12}-$timestamp"
...
for file in "$api_index_src" "$contract_src" "$normalizer_src" "$mongo_config_src" "$compose_override"; do
  test -f "$file"
done
...
run_mongo_mode() {
  local mode="$1"
  docker exec -i \
    -e PUBLIC_CODE_TOOL_CONTRACT_MODE="$mode" \
    -e PUBLIC_CODE_TOOL_CONTRACT_BACKUP_ID="$backup_id" \
