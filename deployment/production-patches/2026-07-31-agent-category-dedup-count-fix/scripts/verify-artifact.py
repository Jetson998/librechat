#!/usr/bin/env python3
"""Verify the immutable Agent category de-dup count Client artifact."""
...
def verify(artifact_zip: Path, metadata_path: Path) -> dict:
    ...
    require(
        provenance["build_environment"] == "independent-build",
        "artifact was not produced by the recorded independent build",
    )
    require(provenance["production_host"] is False, "artifact was built on production")
    require(
        provenance["source_revision"] == source["repository_commit"],
    ...
    with tarfile.open(fileobj=io.BytesIO(tar_payload), mode="r:gz") as archive:
        ...
        for legacy_id, _ in metadata["agent_identity_contract"]["id_mapping"]:
            require(
                re.search(
