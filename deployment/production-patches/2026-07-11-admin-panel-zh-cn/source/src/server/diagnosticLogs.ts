...
const diagnosticLogPageResponseSchema = z.object({
  ...
  nextCursor: z.union([z.string().max(256), z.number().int().min(0)]).nullable(),
...
export const getDiagnosticLogPageFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }: { data: DiagnosticLogFilters }): Promise<DiagnosticLogsResult> => {
    await requireAnyCapability([READ_DIAGNOSTIC_LOGS_CAPABILITY, READ_AUDIT_LOG_CAPABILITY]);
    ...
    return { available: true, ...parseDiagnosticLogPage(await response.json()) };
  });
