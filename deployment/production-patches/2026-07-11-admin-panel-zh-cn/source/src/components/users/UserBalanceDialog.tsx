...
export function UserBalanceDialog({ user, onClose }: t.UserBalanceDialogProps) {
  ...
  const invalidAmount =
    !Number.isFinite(parsedAmount) || parsedAmount <= 0 || parsedAmount > 100000;
  const submit = () => {
    setError('');
    if (invalidAmount) {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex rounded-lg border border-(--cui-color-stroke-default) bg-(--cui-color-background-default)">
            <span className="border-r border-(--cui-color-stroke-default) px-3 py-2 text-sm text-(--cui-color-text-muted)">
              $
            </span>
            <input
              id="balance-amount"
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {balanceQuery.data!.adjustments.map((entry) => (
                  ...
                  >
                    <td className="px-3 py-2 whitespace-nowrap text-(--cui-color-text-muted)">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-(--cui-color-text-default)">
                      {entry.balanceAfterUsd == null ? '-' : formatUsd(entry.balanceAfterUsd)}
                    </td>
                    <td className="px-3 py-2 text-(--cui-color-text-default)">
                      {entry.note || '-'}
                    </td>
                  </tr>
                ))}
