...
source_user="$root_dir/user-usage-dashboard/65ac5e4d0eb0-20260718135751/user.js"
...
for path in \
  ...
  "$source_usage" "$source_user" "$usage_patch/api/usage-dashboard.js" "$usage_patch/api/user.js" \
...
test "$(sha256sum "$source_user" | awk '{print $1}')" = "$expected_user_sha"

node --check "$usage_patch/api/usage-dashboard.js"
node --check "$usage_patch/api/user.js"
...
docker cp "$usage_patch/api/user.js" LibreChat-API:/tmp/lc-cost-detail-user-candidate.js
...
install -m 0444 "$usage_patch/api/user.js" "$release_root/user.js"
...
python3 - "$compose_override" "$candidate_override" "$release_client" "$release_usage" "$release_root/user.js" <<'PY'
import sys
import yaml

source, destination, release_client, release_usage, release_user = sys.argv[1:]
with open(source, encoding="utf-8") as handle:
    data = yaml.safe_load(handle)
api = data.setdefault("services", {}).setdefault("api", {})
volumes = api.setdefault("volumes", [])
managed_targets = (":/app/client/dist:ro", ":/app/api/server/routes/usage-dashboard.js:ro", ":/app/api/server/routes/user.js:ro")
...
volumes.extend([
    ...
    f"{release_user}:/app/api/server/routes/user.js:ro",
])
api["volumes"] = volumes
...
grep -Fq "router.get('/usage-dashboard', requireJwtAuth, configMiddleware, usageDashboardHandler);" "$release_root/user.js"
...
docker exec LibreChat-API grep -Fq "router.get('/usage-dashboard', requireJwtAuth, configMiddleware, usageDashboardHandler);" /app/api/server/routes/user.js
docker cp "$usage_patch/scripts/test-production-aggregation.js" LibreChat-API:/tmp/lc-cost-detail-production-test.js
