...
const officeContextExts = new Set(['.docx', '.xlsx', '.xlsm', '.pptx']);
const officeContextMimeTypes = new Set([
...
const isOfficeContextFile = (file) => {
  const ext = path.extname(file?.originalname ?? '').toLowerCase();
  return officeContextExts.has(ext) || officeContextMimeTypes.has(file?.mimetype);
};

const getOfficeConverterURLs = () => {
  const configured = process.env.OFFICE_CONVERTER_URL;
  if (configured) {
    return configured
      .split(',')
      .map((url) => url.trim())
  ...
  return [
    'http://host.docker.internal:8095/api/convert',
    'http://172.17.0.1:8095/api/convert',
    'http://172.18.0.1:8095/api/convert',
    'http://172.19.0.1:8095/api/convert',
  ];
};

const parseOfficeWithConverter = async ({ file }) => {
  const urls = getOfficeConverterURLs();
  let lastError;
  for (const url of urls) {
    try {
      ...
      const response = await axios.post(url, form, {
        ...
        timeout: Number(process.env.OFFICE_CONVERTER_TIMEOUT_MS || 120000),
        maxContentLength: Infinity,
        maxBodyLength: Infinity,
      });
      const text = response?.data?.markdown || response?.data?.text;
      if (typeof text === 'string' && text.trim()) {
        return {
          text,
          bytes: Buffer.byteLength(text, 'utf8'),
          filepath: response?.data?.filepath || FileSources.document_parser,
          type: 'text/markdown',
        };
      }
      throw new Error(`Office converter at ${url} returned no markdown`);
    } catch (err) {
      lastError = err;
      logger.warn(`[processAgentFileUpload] Office converter failed at ${url}: ${err.message}`);
    }
  }
  throw lastError ?? new Error('Office converter is unavailable');
};
...
const processAgentFileUpload = async ({ req, res, metadata }) => {
  ...
  if (tool_resource === EToolResources.execute_code) {
     ...
     * @return {Promise<void>}
     */
    const createTextFile = async ({ text, bytes, filepath, type = 'text/plain', sourceFile = null }) => {
      ...
      const fileInfo = {
        ...removeNullishValues({
          text,
          textFormat: 'text',
          bytes: sourceFile?.bytes ?? bytes,
          file_id,
          temp_file_id,
          user: req.user.id,
          type: sourceFile?.type ?? type,
          filepath: sourceFile?.filepath ?? filepath ?? file.path,
          ...(sourceFile?.storageMetadata ?? {}),
          source: sourceFile?.source ?? FileSources.text,
          filename: sourceFile?.filename ?? file.originalname,
          model: messageAttachment ? undefined : req.body.model,
          context: messageAttachment ? FileContext.message_attachment : FileContext.agents,
          embedded: sourceFile?.embedded,
          tenantId: req.user.tenantId,
        }),
        ...retentionExpiry,
      };
    ...
    const preserveOriginalOfficeSource = async () => {
      if (!isOfficeContextFile(file)) {
        return null;
      }
      const source = getFileStrategy(appConfig, { isImage: false });
      const { handleFileUpload } = getStrategyFunctions(source);
      const sanitizedUploadFn = createSanitizedUploadWrapper(handleFileUpload);
      const uploaded = await sanitizedUploadFn({
        req,
        file,
        file_id,
        basePath,
      ...
      return {
        ...
        filename: uploaded.filename ?? sanitizeFilename(file.originalname),
        filepath: uploaded.filepath,
        storageMetadata,
        source,
    ...
    const shouldUseOfficeConverter = isOfficeContextFile(file);

    const shouldUseConfiguredOCR =
      !shouldUseOfficeConverter &&
      appConfig?.ocr != null &&
    ...
    const resolveDocumentText = async () => {
      if (shouldUseOfficeConverter) {
        return await parseOfficeWithConverter({ file });
      }
    ...
    if (shouldUseOfficeConverter) {
      ...
      if (ocrResult) {
        const { text, bytes, filepath: ocrFileURL, type } = ocrResult;
        const sourceFile = await preserveOriginalOfficeSource();
        return await createTextFile({ text, bytes, filepath: ocrFileURL, type, sourceFile });
      }
      throw new Error(`Unable to extract text from "${file.originalname}" with the Office converter.`);
    }
