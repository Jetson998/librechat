...
async function loadToolsForExecution({
  ...
  const claudeCodeBashAliases = new Set(['Bash', 'bash', 'PowerShell', 'powershell']);
  const bashToolRequestName = toolNames.find(
    (name) => name === AgentConstants.BASH_TOOL || claudeCodeBashAliases.has(name),
  );
  const ptcToolNames = [
    AgentConstants.BASH_PROGRAMMATIC_TOOL_CALLING,
    AgentConstants.PROGRAMMATIC_TOOL_CALLING,
  ...
  const isBashToolRequested = !!bashToolRequestName;
