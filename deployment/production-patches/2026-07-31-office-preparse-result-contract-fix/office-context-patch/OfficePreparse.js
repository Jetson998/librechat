...
const extractFirstJSONObject = (source) => {
  ...
  if (source[index] !== '{') {
    throw new SyntaxError('manifest payload did not start with a JSON object');
  }
  ...
  throw new SyntaxError('manifest JSON object was incomplete');
};

const classifyTrailingOutput = (trailing) => {
  const trimmed = trailing.trim();
  if (!trimmed) return 'none';
  if (trimmed.includes(MANIFEST_MARKER)) return 'duplicate-manifest';
  if (/^(?:stderr:|Generated files:|Available files \([^\n]*\):)/i.test(trimmed)) {
    return 'tool-wrapper';
  }
  if (/^(?:(?:\/[^\n]+|<string>):\d+:\s*)?[A-Za-z]*Warning:/i.test(trimmed)) {
    return 'runtime-warning';
  }
  return 'unexpected';
};

const parseManifestFromToolContent = (content) => {
  const markerIndex = content.indexOf(MANIFEST_MARKER);
  if (markerIndex < 0) throw new Error('Office pre-parse returned no manifest.');

  const extracted = extractFirstJSONObject(
    content.slice(markerIndex + MANIFEST_MARKER.length),
  );
  const trailingKind = classifyTrailingOutput(extracted.trailing);
  if (trailingKind === 'duplicate-manifest') {
    throw new SyntaxError('tool output contained more than one manifest marker');
  }
  if (trailingKind === 'unexpected') {
    throw new SyntaxError('manifest was followed by unexpected tool output');
  }

  return {
    manifest: JSON.parse(extracted.json),
    trailingKind,
    trailingLength: extracted.trailing.trim().length,
  };
};
...
const createOfficePreparse = ({ createBashExecutionTool, getCodeApiAuthHeaders, logger }) => ({
  prepareCurrentTurnOfficeContext: async ({
    ...
    let manifest;
    try {
      const parsed = parseManifestFromToolContent(content);
      manifest = parsed.manifest;
      if (parsed.trailingLength > 0) {
        logger.debug('[office-preparse] ignored non-manifest tool wrapper output', {
          trailingKind: parsed.trailingKind,
          trailingLength: parsed.trailingLength,
          contentPartCount: Array.isArray(result?.content) ? result.content.length : 1,
        });
      }
    } catch (error) {
