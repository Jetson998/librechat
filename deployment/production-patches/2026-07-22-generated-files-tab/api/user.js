...
const { createGeneratedFilesHandler } = require('./generated-files');

const settings = require('./settings');

const router = express.Router();
const usageDashboardHandler = createUsageDashboardHandler({ mongoose, logger });
const generatedFilesHandler = createGeneratedFilesHandler({ mongoose, logger });

router.use('/settings', settings);
router.get('/', requireJwtAuth, getUserController);
router.get('/usage-dashboard', requireJwtAuth, configMiddleware, usageDashboardHandler);
router.get('/generated-files', requireJwtAuth, generatedFilesHandler);
