...
release_dir="/opt/librechat/fable-tool-alias/${source_revision:0:12}-$timestamp"
compose_dir="/opt/librechat"
compose_override="$compose_dir/compose.override.yaml"
compose_backup="$compose_override.bak-$timestamp"
backup_id="fable-tool-alias-${source_revision:0:12}-$timestamp"
expected_baseline="b9379a4ee7f5f6d7bca4d99d88a8aa70b9e0dcbe4aae35dbe1393b55f6ea86de"
...
for file in "$api_index_src" "$normalizer_src" "$mongo_config_src" "$compose_override"; do
  test -f "$file"
done
...
run_mongo_mode() {
  local mode="$1"
  docker exec -i \
    -e FABLE_TOOL_ALIAS_MODE="$mode" \
    -e FABLE_TOOL_ALIAS_BACKUP_ID="$backup_id" \
