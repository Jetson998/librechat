...
可创建、可返回的“智能助手”工作区，并建立可重复的 LibreChat Client 源码构建链。
...
1. 用户界面统一使用“智能助手”，内部 API、类型、权限和数据库继续使用 `agent`。
...
统一侧栏只保留一个面向用户的“智能助手”入口。点击后进入 `/agents`，不再要求用户
...
“智能助手”虽然打开主区域路由，但必须遵守统一侧栏与其他菜单相同的交互状态机：

- `/agents`、`/agents/:category` 以及带查询参数的工作区 URL 必须让“智能助手”显示灰底选中态；
...
1. 只有满足现有 Agent/Marketplace 权限时才显示“智能助手”入口；
2. `/agents` 默认进入“推荐助手”；
...
17. `/agents` 直达、刷新和分类深链接时“智能助手”具有灰底选中态；
