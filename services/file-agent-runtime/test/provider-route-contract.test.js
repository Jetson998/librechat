...
import { loadProviderRoutes } from '../src/provider-route-registry.js';
...
test('Runtime loads multiple explicit provider routes and their server-side keys', async () => {
  ...
  try {
    ...
    assert.equal(routes[0].providerRouteRef, 'custom:Muskapis-openai');
    assert.equal(routes[0].apiKey, 'openai-secret-value');
    assert.equal(routes[1].protocol, 'anthropic-messages');
    assert.equal(routes[1].apiKey, 'anthropic-secret-value');
  } finally {
