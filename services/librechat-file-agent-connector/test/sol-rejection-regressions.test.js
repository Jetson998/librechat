...
function officeContext({ file, instruction }) {
  return {
    req: {
...
test('natural-language XLSX to PPT request is classified as Office Compose before acceptance parsing', () => {
  const profile = productionCapabilityProfile({
    files: [{ file_id: 'xlsx-1', filename: '2026-data.xlsx', type: XLSX_MIME }],
    instruction: '根据这个 Excel 生成一页 API 模型来源说明 PPT',
  });
...
test('PPTX title-edit language routes to the PPTX profile and does not require an XLSX resolver', async () => {
  ...
  const preflight = await createProductionOfficePreflight({
    allowlistedUserIds: new Set(['user-1']),
  })(officeContext({ file, instruction }));
